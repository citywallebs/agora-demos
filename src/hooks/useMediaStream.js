...
export default function useMediaStream(client,filter){
  ...
  useEffect(()=>{
    ...
    const removeRemote = (evt) => {
      ...
      if (stream) {
        ...
        }else if(remoteStreamList.length===0){
          setRemoteStreamList([])
        }
      }
    };
  ...
  return [localStream,remoteStreamList,[localStream,...remoteStreamList],setLocalStream]
}